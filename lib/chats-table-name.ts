 ...
 */
// Extension-ful import so `node --test` can run this module.
...
/**
 * The column holding the lead's id in the CRM, named after the CRM whose flow
 * writes it. Kommo's flows have written `id_de_kommo` since the beginning;
 * Go High Level's write `id_crm`. Nothing renames an existing table: a client
 * keeps the column its flow already fills.
 */
export const LEAD_ID_COLUMN: Record<Crm, string> = {
  kommo: "id_de_kommo",
  ghl: "id_crm",
};
 ...
 * for reporting. Both already exist in that database, and only there: a client
 * running its own Postgres has neither, and granting to a role that does not
 * exist aborts the whole script, so `grants: false` leaves them out.
 *
 * The lead's id column is named after the CRM the flow talks to, because the
 * flow is what writes it: a Kommo flow writes `id_de_kommo`, a Go High Level
 * one writes `id_crm`. The Go High Level column is UNIQUE, since that flow
 * saves a turn with `insert ... on conflict (id_crm) do update`, and an
 * ON CONFLICT target without a unique index is a runtime error on every
 * message. Reading either shape is lib/db/chats-history's problem, not the
 * table's.
 */
export function buildCreateChatsTableSql(
  schemaName: string,
  opts: { grants?: boolean; crm?: Crm } = {},
): string {
  const q = quoteIdent(schemaName);
  const ghl = opts.crm === "ghl";
  const leadId = LEAD_ID_COLUMN[opts.crm ?? "kommo"];
  const index = ghl
    ? `create unique index if not exists chats_crm_idx on ${q}.${CHATS_TABLE} (${leadId});`
    : `create index if not exists chats_kommo_idx on ${q}.${CHATS_TABLE} (${leadId});`;
  const grants =
    opts.grants === false
      ? ""
      : `
...
create table if not exists ${q}.${CHATS_TABLE} (
  ...
  ${leadId.padEnd(18)} text not null,
  historial          text,
  turnos             jsonb,
...
create trigger turnos_normalize before insert or update on ${q}.${CHATS_TABLE}
  for each row execute function public.normalize_turnos();${grants}`;
}
